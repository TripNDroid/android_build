...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^tripndroid_") ; then
       CM_BUILD=$(echo -n $1 | sed -e 's/^tripndroid_//g')
    else
       CM_BUILD=
    fi
    export CM_BUILD
...
function breakfast()
{
    ...
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/tripndroid/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            lunch tripndroid_$target-userdebug
        fi
    fi
    return $?
}
...
function eat()
{
    if [ "$OUT" ] ; then
        ...
        ZIPFILE=tripndroid-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.tripndroid.device=$CM_BUILD");
    then
